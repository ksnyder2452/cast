...
public class TablesModel : PageModel
{
    ...
    public static string messagebus_home = new ConfigurationBuilder().AddJsonFile("appsettings.json").Build().GetSection("AppSettings")["message_bus_home"];
    ...
    public static string connectString = "Server=" + mysql_Server + "; Database=" + mysql_Database + "; Uid=" + mysql_User + "; Pwd=" + mysql_Password + "; Port=3306";
    ...
    static string select_framework_info = "select reference_uuid, display_name from logger where message like 'Started Test Client Service%' and display_name NOT LIKE 'SETUP New Test Framework - IGNORE THIS ENTRY' and filter_on_group like '%' and filter_on_owner like '%' and filter_on_location like '%' and filter_on_keyword like '%' and virtual_delete = 0 order by order_in_system DESC";
    string pre_select_framework_info = "select reference_uuid, display_name from logger where message like 'Started Test Client Service%' and display_name NOT LIKE 'SETUP New Test Framework - IGNORE THIS ENTRY'";
    ...
    ConnectionFactory factory = new ConnectionFactory { HostName = messagebus_home };
    ...
    public IActionResult OnGet(string param2)
    {
        if (param2 != null)
        {
            if (param2.StartsWith("schedule_") && !scheduleList.Contains(param2.Substring(9)))
            {
                ...
                string insertSchedule = "insert into testsuite_state(uuid, reference_uuid, state, event_time_dt, scheduled_time) values('" + startmyuuidAsString + "', '" + referenceUUID + "', 'SCHEDULED', NOW(), '" + scheduledTime + "')";
        ...
        else
        {
            ...
            using (MySqlConnection conn = new MySqlConnection(connectString))
            {
                ...
                string selectUtafState = "select state, name from utaf_state_tracker";
                serviceState.Clear();
                serviceName.Clear();
                using (MySqlCommand command = new MySqlCommand(selectUtafState, conn))
                {
                    MySqlDataReader rdr = command.ExecuteReader();
                ...
                string selectFrameworkNames = "select distinct(filter_on) from logger";
                filterFrameworks.Clear();
                filterFrameworks.Add("All");
                using (MySqlCommand command = new MySqlCommand(selectFrameworkNames, conn))
                {
                    ...
                    while (rdr.Read())
                    {
                        ...
                        {
                            filterFrameworks.Add((string)rdr[0]);
                        }
                    }
                    rdr.Close();
                }
                filterFrameworks.RemoveAll(item => item == null);
                filterFrameworks.RemoveAll(item => item == "");

                selectFrameworkNames = "select distinct(filter_on_group) from logger";
                ...
                string selectCustomActions = "select name, description from custom_actions";
                ...
                foreach (string originator in mysqlDictionary.Keys)
                {
                    ...
                    string selectState = "select state, event_time_dt from testsuite_state where reference_uuid = '" + originator + "' and order_in_system = (select MAX(order_in_system) from testsuite_state where reference_uuid = '" + originator + "' and virtual_delete = 0)";
                    ...
                    string selectFrameworkFunctionality = "select start_supported, stop_supported, pause_supported, resume_supported, abort_supported, restart_supported from framework_functionality where reference_uuid = '" + originator + "'";
                    ...
                    else if (currentStateStr.ToUpper().StartsWith("COMPLETED TESTSUITE") || (currentStateStr.ToUpper().StartsWith("TESTSUITE ") && currentStateStr.ToUpper().EndsWith(" WAS STOPPED")) || (currentStateStr.ToUpper().StartsWith("TESTSUITE ") && currentStateStr.ToUpper().EndsWith(" WAS ABORTED")))
                    {
                        if (restartRunEnabled)
                        {
                            startRunStr = "yes";
    ...
    public async Task<IActionResult> OnPostMyAction2(string id)
    {
        ...
        string message = "message for test_client_service_" + clientUUID + ": local: action: start test run";
    ...
    public async Task<IActionResult> OnPostMyAction3(string id)
    {
        ...
        //Prepend string test_client_service_ to reference_uuid to build the originator
        string message = "message for test_client_service_" + clientUUID + ": local: action: stop test run";
        var body = Encoding.UTF8.GetBytes(message);
        await channel.BasicPublishAsync(exchange: string.Empty, routingKey: "test_execution_service", body: body);
        return RedirectToPage("./tables");
    }
    ...
    public async Task<IActionResult> OnPostMyAction4(string id)
    {
        ...
        //Prepend string test_client_service_ to reference_uuid to build the originator
        string message = "message for test_client_service_" + clientUUID + ": local: action: pause test run";
        var body = Encoding.UTF8.GetBytes(message);
        await channel.BasicPublishAsync(exchange: string.Empty, routingKey: "test_execution_service", body: body);
        return RedirectToPage("./tables");
    }
    ...
    public async Task<IActionResult> OnPostMyAction5(string id)
    {
        ...
        //Prepend string test_client_service_ to reference_uuid to build the originator
        string message = "message for test_client_service_" + clientUUID + ": local: action: resume test run";
        var body = Encoding.UTF8.GetBytes(message);
        await channel.BasicPublishAsync(exchange: string.Empty, routingKey: "test_execution_service", body: body);
        return RedirectToPage("./tables");
    }
    ...
    public async Task<IActionResult> OnPostMyAction6(string id)
    {
        ...
        //Prepend string test_client_service_ to reference_uuid to build the originator
        string message = "message for test_client_service_" + clientUUID + ": local: action: abort test run";
        var body = Encoding.UTF8.GetBytes(message);
        await channel.BasicPublishAsync(exchange: string.Empty, routingKey: "test_execution_service", body: body);
        return RedirectToPage("./tables");
    }
    ...
    public async Task<IActionResult> OnPostMyAction7(string id)
    {
        ...
        //Prepend string test_client_service_ to reference_uuid to build the originator
        string message = "message for test_client_service_" + clientUUID + ": local: action: restart test run";
        var body = Encoding.UTF8.GetBytes(message);
        await channel.BasicPublishAsync(exchange: string.Empty, routingKey: "test_execution_service", body: body);
        return RedirectToPage("./tables");
    }
    ...
    {
        var factory = new ConnectionFactory { HostName = messagebus_home };
        factory.UserName = rabbitmq_user;
        factory.Password = rabbitmq_pwd;
        using var connection = await factory.CreateConnectionAsync();
        using var channel = await connection.CreateChannelAsync();
